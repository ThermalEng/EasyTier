...
impl HealthChecker {
    ...
    fn update_memory_record(
        ...
        if let Some(mut record) = self.node_records.get_mut(&node_id) {
            ...
            self.node_records.insert(node_id, new_record);
        }
    }
    ...
    async fn test_node_healthy(
        inst_id: uuid::Uuid,
        instance_mgr: Arc<NetworkInstanceManager>,
        // return version and response time on healthy
    ...
    async fn node_health_check_task(
        ...
        loop {
            ...
            match Self::test_node_healthy(inst_id, instance_mgr.clone()).await {
                Ok((version, response_time)) => {
                    if let Err(e) =
                        NodeOperations::update_node_status(&db, node_id, true, None).await
                    {
