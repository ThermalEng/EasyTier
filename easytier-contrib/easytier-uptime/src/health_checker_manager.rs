...
impl HealthCheckerManager {
    ...
    pub async fn start_monitoring(&self) -> anyhow::Result<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                {
                    eprintln!("Error checking node changes: {}", e);
                }
                ticker.tick().await;
            }
        });
    ...
    /// 停止监控指定节点（手动操作）
    pub async fn stop_monitoring_node(&self, node_id: i32) -> anyhow::Result<()> {
        self.health_checker.remove_node(node_id).await?;
        self.current_nodes.write().await.remove(&node_id);
        println!("Manually stopped monitoring node {}", node_id);
        Ok(())
    }

    /// 开始监控指定节点（手动操作）
    pub async fn start_monitoring_node(&self, node_id: i32) -> anyhow::Result<()> {
        self.health_checker.add_node(node_id).await?;
        self.current_nodes.write().await.insert(node_id);
        println!("Manually started monitoring node {}", node_id);
        Ok(())
    }
