    ...
    <el-card class="nodes-card">
      ...
      <el-table :data="nodes" v-loading="loading" stripe style="width: 100%" row-key="id">
        ...
        <el-table-column prop="address" label="节点连接地址" width="250">
          ...
          <template #default="{ row }">
            <el-tag type="primary" size="" style="margin-bottom: 0.2rem;" @click="copyAddress(apiUrl + row.id)"> {{
              apiUrl
            }} {{ row.id }}</el-tag>
      ...
      <div v-if="selectedNode" class="node-details">
        ...
        <div class="health-stats" v-if="healthStats">
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="6">
              <div class="health-stat-item">
                <div class="stat-value">{{ healthStats.avg_response_time || 0 }}ms</div>
...
const totalConnections = computed(() =>
  nodes.value.reduce((sum, node) => sum + node.current_connections, 0)
)
